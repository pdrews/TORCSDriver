...
const float Driver::FULL_ACCEL_MARGIN = 2.0f;       // [m/s] Margin reduce oscillation of brake/acceleration.
...
void Driver::initTrack(tTrack* t, void *carHandle, void **carParmHandle, tSituation *s)
{
  ...
  */
  if (*carParmHandle == NULL) {
    snprintf(buffer, BUFSIZE, "drivers/bt//default.xml", INDEX);
    *carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
    }
...
void Driver::drive(tSituation *s, tCarElt* car, float splinePos)
{
  ...
  if (isStuck()) {
    ...
    car->_steerCmd = getSteer(splinePos);
...
float Driver::getBrake(float trajectoryPosition)
{
  ...
  if (car->_speed_x < -MAX_UNSTUCK_SPEED) {
    ...
    float mu = car->_trkPos.seg->surface->kFriction*TIREMU*MU_FACTOR;
    float maxCarAcceleration = mu * G;
   // float maxlookaheaddist = currentspeedsqr/(2.0f*mu*G);
    //float lookaheaddist = getDistToSegEnd();
    float currentlookahead = MAX_LOOKAHEAD_DIST + trajectoryPosition;
    ...
    while (currentlookahead > trajectoryPosition) {
      ...
      //float centripetalAcceleration = (double)allowedspeed * (double)allowedspeed * ((double)MIN(currentTrajectory.getCurvature(currentlookahead),(double)0.000001) + (double)((double)1.0 / (double)trackCurve));
      ...
      if (allowedspeed > 100)
        continue;
      float dv = 1/allowedspeed * 
                 sqrt((mu*G)*(mu*G) - 
                     ((totalCurvature * (allowedspeed*allowedspeed))*
                     (totalCurvature * (allowedspeed*allowedspeed)))) *
                 LOOKAHEAD_STEP;
      //cout << "dv " << dv << " one " << (mu*G)*(mu*G) << " two " << 
      //  (totalCurvature - (allowedspeed*allowedspeed)) << endl;

      //float vprev = sqrt(allowedspeed * allowedspeed + 
      //    2 * maxBrakeAccel * LOOKAHEAD_STEP);
      allowedspeed = allowedspeed + dv;
...
double Driver::getCurve(float dist)
{
  ...
  //cout << "distance " << dist << " startdist " << sing.wrap.m_trackData[i].first;
  while(i < sing.wrap.m_trackData.size() && dist > sing.wrap.m_trackData[i].first)
  {
    i ++;
    //cout << " i " << i;
  }
  //cout << " val " << sing.wrap.m_trackData[i-1].second;
  //cout << endl;
  double ret = sing.wrap.m_trackData[i-1].second;
  if (ret == 0) ret = 5000;
  return fabs(ret);
}
...
// Compute steer value.
float Driver::getSteer(float splinePos)
{
  float targetAngle;
  vec2f target = getTargetPoint(splinePos);
...
vec2f Driver::getTargetPoint(float carPos)
{
  ...
  float offset = currentTrajectory.computeSplineValue(carPos + lookahead);
        ...
        //Here we need to get the x y position of this point on the spline
